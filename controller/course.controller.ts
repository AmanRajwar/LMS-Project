...
export const addReview = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        // if the courseID already exists the the user userCourseList based on _id
        const courseExists = userCourseList?.some((course: any) => course._id.toString() === courseId.toString())

        if (!courseExists) {
            return next(new ErrorHandler("Your are not eligible to access this course", 500))
        }
        const course = await CourseModel.findById(courseId);
        ...
        if (course) {
            course.ratings = avg / course.reviews.length;// one example we have 2 reviews one is 5 another one 4 so the math working like this  = 9/2 = 4.5 ratings
        }
